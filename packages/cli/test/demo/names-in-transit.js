import { checkpoint as resume } from "./sending-messages.js";

export async function section(testLine, execa) {
  await resume(execa);

  // In this example, we send alice our "doubler" but let it appear...
  await testLine(execa`endo send alice ${'Please enjoy this @counter:doubler.'}`);
...
export async function checkpoint(execa) {
  await resume(execa);
